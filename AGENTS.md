...
- AI 编排：NanoClaw（Codex Agent SDK，工作流 + 工具调用）+ 自建 sqlite-vec RAG；Dify 仅剩 PRD 生成 Chatflow
...
1. **交互层** — NanoClaw（飞书 AI 工作台，Codex Agent SDK）+ ArcFlow Web（Vue 3 管理界面）
2. **通知层** — 飞书（状态推送，消息卡片含 Plane 跳转 / 审批链接）
3. **编排层** — NanoClaw（Codex Agent SDK，工具调用 + 工作流）+ Gateway 内置 sqlite-vec RAG；Dify 仅残留 PRD Chatflow
4. **衔接层** — 胶水服务 Gateway（Webhook 路由、Git 读写、Codex 调度、NanoClaw dispatch、飞书通知、RAG）
...
→ NanoClaw 工作流（Codex Opus 生成技术设计 → Codex Sonnet 生成 OpenAPI）
→ 写回 docs Git → 飞书消息卡片通知研发 Review（卡片含 Plane 跳转 + 审批 token）
→ 通过后进入两轮代码生成：
  第一轮：后端代码生成 → 研发 Review MR → 合并
  第二轮：设计师出 Figma 设计稿 → Codex 通过 Figma MCP 生成 UI 代码 → 研发 Review / 微调
...
| 场景 | 模型 |
|------|------|
| PRD → 技术设计文档 | Codex-opus-4-6 |
| 技术文档 → OpenAPI | Codex-sonnet-4-6 |
| CI 日志分析 / Bug 报告 | Codex-sonnet-4-6 |
| RAG 知识库问答 | Codex-sonnet-4-6（Gateway 自建 sqlite-vec）|
| 代码生成 / Bug 修复 | Codex headless (Codex-sonnet-4-6) |
| NanoClaw 对话 | Codex-sonnet-4-6 (Agent SDK) |

## 关键设计决策

- 选 Markdown + Git 而非飞书文档：AI 零转换损耗，版本追踪清晰
- 选 Plane 而非 Jira：原生 MCP Server，Codex 可直接读写 Issue
...
- Codex 通过 Figma MCP Server 读取设计稿生成 UI 代码
...
| `2026-04-02-Codex-md-specs-design.md` | 各端 AGENTS.md 规范（5 个仓库） |
...
| `2026-04-03-Codex-github-workflow-guide-design.md` | Codex GitHub 工作流指南 |
