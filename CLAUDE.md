...
ArcFlow 是一个 AI 研发运营一体化平台，以 Markdown + Git 为数据底座、AI 为执行引擎，串联从 PRD 到代码生成的全流程。当前 Gateway + Web + Dify + Plane + NanoClaw 已打通，处于端到端联调与 NanoClaw 能力扩展阶段。
...
4. **衔接层** — 胶水服务 Gateway（Webhook 路由、Git 读写、Claude Code 调度、NanoClaw dispatch、飞书通知）
5. **数据层** — docs Git + 代码仓库 + Plane Issue + Weaviate

Dify 负责 AI 工作流编排，Gateway 负责系统间数据搬运与统一 API 出入口，两者职责分离。
...
→ Dify 工作流（Claude Opus 生成技术设计 → Claude Sonnet 生成 OpenAPI）
