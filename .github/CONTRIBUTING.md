...
外部贡献者通过 Fork 提交的 PR，出于安全和成本考虑，CI / AI Review **不会自动触发**。

**流程**：

1. 外部贡献者 Fork 仓库并提交 PR
2. Maintainer 人工审查 PR 代码，确认无恶意内容
3. Maintainer 给 PR 打上 `approved-for-ci` 标签
4. CI 和 AI Review 自动触发
5. 如果 PR 有新的 push，需要重新审查并再次打标签

> **注意**：`approved-for-ci` 标签仅授权 CI 运行，不代表代码被 Approve。正常的 Code Review 流程仍需进行。
